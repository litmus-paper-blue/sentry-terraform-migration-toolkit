...
class SentryDiscovery:
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                ...
                if attempt < self.retry_attempts:
                    ...
                    continue
                raise SentryAPIError(f"Request timeout after {self.retry_attempts} retries")
                ...
                if attempt < self.retry_attempts:
                    ...
                    time.sleep(wait_time)
                    continue
                raise SentryAPIError(f"Connection error after {self.retry_attempts} retries")
            
            except requests.exceptions.RequestException as e:
                raise SentryAPIError(f"Request failed: {str(e)}")
        
        raise SentryAPIError(f"Failed to make request after {self.retry_attempts + 1} attempts")
    ...
    def get_team_members(self, org_slug: str, team_slug: str) -> List[Dict[str, Any]]:
        ...
        data = self._make_request(f'/organizations/{org_slug}/teams/{team_slug}/members/')
        if not isinstance(data, list):
            return []
    ...
    def get_project_teams(self, org_slug: str, project_slug: str) -> List[Dict[str, Any]]:
        """Get teams assigned to a specific project"""
        logger.debug(f"Fetching teams for project: {org_slug}/{project_slug}")
        
        data = self._make_request(f'/organizations/{org_slug}/projects/{project_slug}/teams/')
        if not isinstance(data, list):
            return []
    ...
    def discover_all(
        ...
        if not teams_only:
            ...
            result['projects'] = [self._serialize_project(project) for project in projects]
